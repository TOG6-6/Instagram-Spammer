...
#api end - just to inspect the api, this is useless for the user and you can even remove it
...
def main(): #program
    ...
    os.system('color 3f')
    os.system('cls')
    ctypes.windll.kernel32.SetConsoleTitleW("Welcome To Instagram Bomber! Made By TOG With <3")
    print(f"""{Fore.BLUE}
██╗███╗░░██╗░██████╗████████╗░█████╗░░██████╗░██████╗░░█████╗░███╗░░░███╗
██║████╗░██║██╔════╝╚══██╔══╝██╔══██╗██╔════╝░██╔══██╗██╔══██╗████╗░████║
██║██╔██╗██║╚█████╗░░░░██║░░░███████║██║░░██╗░██████╔╝███████║██╔████╔██║
...
╚═════╝░░╚════╝░╚═╝░░░░░╚═╝╚═════╝░╚══════╝╚═╝░░╚═╝
""")
    username = input(f"{Fore.MAGENTA}Enter Your Instagram Username: ")
    passwords = input(f"{Fore.MAGENTA}Enter Your Instagram Password: ")
    ctypes.windll.kernel32.SetConsoleTitleW(f"Instagram Username -> {username} | Instagram Password -> {passwords}")
    try:
        api = InstagramAPI(username, passwords)
        api.login()
        os.system('color 2f')
        ctypes.windll.kernel32.SetConsoleTitleW(f"Instagram Username -> {username} | Instagram Password -> {passwords} | LOGGED IN!")
        print(Fore.GREEN + f"Successfully Logged Into {username} with password -> {passwords}")
        pass
    except Exception as e:
        ctypes.windll.kernel32.SetConsoleTitleW("Failed To Login!")
    ...
    os.system('cls')
    os.system('color 3f')
    user = input(Fore.RED + "Enter Victim's Username: ")
    try:
        response = requests.get("https://www.instagram.com/"+user+"/?__a=1")
        respJSON = response.json()
        user_id = str(respJSON['graphql'].get("user").get("id"))
        print(Fore.GREEN + f"Successfully Found User -> {user}")
        ctypes.windll.kernel32.SetConsoleTitleW(f"Successfully Found User -> {user}")
        pass
    except:
        print(Fore.RED + f"Unknown Victim -> {user}!")
        os.system('color 4f')
        ctypes.windll.kernel32.SetConsoleTitleW(f"Unknown Victim -> {user}!")
        time.sleep(3)
        os.system('cls')
        main()
    time.sleep(2)
    os.system('cls')
    os.system('color 3f')
    message = input(Fore.CYAN + "Enter The Message To Spam: ")
    ctypes.windll.kernel32.SetConsoleTitleW(f"Message To Spam -> {message}")
    amt = int(input(Fore.CYAN + "How Many Times Do You Want The Message To Spam?: "))
    ctypes.windll.kernel32.SetConsoleTitleW(f"Number Of Times To Spam The Message -> {amt}")
    delay = float(input(Fore.CYAN + "How Many Seconds Of Delay Do You Want Per Message?: "))
    ctypes.windll.kernel32.SetConsoleTitleW(f"Delay -> {delay}")
    count = 0
    try:
        os.system('color 3f')
        print(Fore.GREEN + f"Spamming -> {user} | Message -> {message} | Count -> 0/{amt}")
        print(Fore.LIGHTBLUE_EX + "How Many Times Has The Message Been Spammed? Check The Title Of The Program XD!")
        for i in range(amt):
            api.sendMessage(user_id,message)
            count += 1
            ctypes.windll.kernel32.SetConsoleTitleW(f"Number Of Times Message Spammed -> {count}/{amt} | Made With Love By TOG!")
        print(Fore.GREEN + "Done!")
        print(Fore.GREEN + f"Spammed -> {user} | Message -> {message} | Count -> {amt}/{amt}")
        time.sleep(3)
        os.system('cls')
        main()
    except Exception as e:
        os.system('color 4f')
        ctypes.windll.kernel32.SetConsoleTitleW(f"Error Occured -> {e}")
        print(Fore.RED + f"Error Occured: {e}")
        time.sleep(3)
        os.system('cls')
        main()
